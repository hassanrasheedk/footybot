...
void myFunc(Mat &cameraFeed, Entity &playerfront, Entity &playerrear, Entity &ball, Entity &goal);
...
//void drawObject(vector<Entity> theEntities, Mat &frame) {
//
//	for (int i = 0; i < theEntities.size(); i++) {
//		cv::circle(frame, cv::Point(theEntities.at(i).getXPos(), theEntities.at(i).getYPos()), 10, cv::Scalar(0, 0, 255));
//		cv::putText(frame, intToString(theEntities.at(i).getXPos()) + " , " + intToString(theEntities.at(i).getXPos()), cv::Point(theEntities.at(i).getXPos(), theEntities.at(i).getYPos() + 20), 1, 1, Scalar(0, 255, 0));
//		cv::putText(frame, theEntities.at(i).getType(), cv::Point(theEntities.at(i).getXPos(), theEntities.at(i).getYPos() - 30), 1, 2, theEntities.at(i).getColor());
//	}
//}
void drawObject(Entity anEntity, Mat &frame) 
{
...
void trackFilteredObject(Entity &anEntity,Mat threshold, Mat HSV, Mat &cameraFeed) {
...
void myFunc(Mat &cameraFeed,Entity &playerfront, Entity &playerrear, Entity &ball, Entity &goal)
{
	...
	morphOps(threshold);
	trackFilteredObject(playerfront, threshold, HSV, cameraFeed);//passes the object to the tracking function													 //playerrear
	cvtColor(cameraFeed, HSV, COLOR_BGR2HSV);
...
bool case1(Entity PF, Entity PR)
{
	if (PF.getXPos() > PR.getXPos()&&PF.getYPos() > PR.getYPos())
		return 1;
}
bool case2(Entity PF, Entity PR)
{
	if (PF.getXPos() > PR.getXPos()&&PF.getYPos() < PR.getYPos())
		return 1;
}
bool case3(Entity PF, Entity PR)
{
	if (PF.getXPos() < PR.getXPos()&&PF.getYPos() > PR.getYPos())
		return 1;
}
bool case4(Entity PF, Entity PR)
{
	if (PF.getXPos() < PR.getXPos()&&PF.getYPos() < PR.getYPos())
		return 1;
}
...
//	if (balld == "NORTHWEST")
//	{
//		cout << balld << " " << playerrear.getXPos() << " " <<playerrear.getYPos() << " " <<ball.getXPos()<<" "<<ball.getYPos()<<endl;
//		while (!((playerrear.getXPos() - playerfront.getXPos() > 0) && (playerfront.getYPos() - playerrear.getYPos()<10)))
//		{
//			myData("L", serial);
//			capture.read(cameraFeed);
//			myFunc(&cameraFeed, playerfront, playerrear, ball, goal);
...
//			imshow(windowName2, cameraFeed);
//		}
//		while (!((playerrear.getYPos() - playerfront.getYPos() > 0) && (playerfront.getXPos() - playerrear.getXPos()<10)))
//		{
//			myData("R", serial);
//			capture.read(cameraFeed);
//			myFunc(&cameraFeed,  playerfront, playerrear, ball, goal);
...
//			imshow(windowName2, cameraFeed);
//		}
//		while (!((playerfront.getYPos() - playerrear.getYPos() > 0) && (playerfront.getXPos() - playerrear.getXPos()<10))) {
//			myData("L", serial);
//			capture.read(cameraFeed);
//			myFunc(&cameraFeed,  playerfront, playerrear, ball, goal);
//			imshow(windowName2, cameraFeed);
